...
function isChannelPaused(channelId) {
  return pausedChannels.has(channelId);
}

function setChannelPaused(channelId, paused) {
  if (paused) pausedChannels.add(channelId);
  else pausedChannels.delete(channelId);
}
...
const SLASH_COMMANDS = [
  ...
  { name: 'pause', description: 'Pause Discord responses for this channel' },
  { name: 'resume', description: 'Resume Discord responses for this channel' },
...
if (DISCORD_PLUGIN_ENABLED) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (command === 'pause' || command === 'resume') {
      setChannelPaused(interaction.channelId, command === 'pause');
      await interaction
        .reply({
          content: `✓ ${command}`,
          flags: [MessageFlags.Ephemeral],
    ...
    if (content.startsWith('/')) {
      ...
      const cappedArgs =
        command === 'delete' && config.triggerPrefix
          ...
          : args;
      if (command === 'pause' || command === 'resume') {
        setChannelPaused(message.channel.id, command === 'pause');
        return;
      }
      dispatchCommand(
        'discord',
...
async function sendText(channelId, text) {
  if (isChannelPaused(channelId)) return;
  const channel = client.channels.cache.get(channelId);
  if (!channel) return;
...
async function sendImages(channelId, images, caption) {
  ...
  enqueue(channelId, () => {
    if (isChannelPaused(channelId)) return;
    return sendImagesToChannel(channel, images, caption);
  });
}
...
async function streamChunk(channelId, payload) {
  ...
  scheduleEdit(session, channel, streamId, () => !isChannelPaused(channelId));
}
